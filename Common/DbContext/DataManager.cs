...
namespace Common.DbContext
{
    public class MySqlConnection : AppConfiguration, IDisposable
    {
        ...
        public System.Data.SqlClient.SqlConnection _Connection; // Other managed resource this class uses.
    ...
    {
        private IntPtr handle; // Pointer to an external unmanaged resource.
        private readonly System.Data.SqlClient.SqlConnection _Connection;      // Other managed resource this class uses.
        private System.Data.SqlClient.SqlDataAdapter _MyDataAdaptor;
        private readonly System.Data.SqlClient.SqlCommand _sqlCommand;
        private bool disposed = false;  // Track whether Dispose has been called.

        // Implement IDisposable.
        // Do not make this method virtual.
        // A derived class should not be able to override this method.
        public MySqlCommand(System.Data.SqlClient.SqlConnection MyConnection)
        {
        ...
        /// <param name="Value">Parameter Value</param>
        public void Add_Parameter(string ParameterName, System.Data.SqlDbType DataType, object Value)
        {
            try
            {
                if (ParameterName.IndexOf("@") < 0)
        ...
        public void Add_Transaction(System.Data.SqlClient.SqlTransaction MyTransaction)
        {
            try
            {
        ...
        private bool Execute_Query_WithTransaction(string Query, System.Data.CommandType CmdType,
            System.Data.SqlClient.SqlTransaction MyTransaction,
            bool UseTransaction)
        {
            try
            {
        ...
        /// <param name="CommandType">Specify command type as Text if you passed Text as Query or 
        /// StoredProcedure if you passed name of Stored Procedure as Query</param>
        public bool Execute_Query(string Query, System.Data.CommandType CommandType)
        {
            try
            {
        ...
        public DataTable Select_Table(string Query, System.Data.CommandType CommandType)
        {
            ...
            try
            {
                ...
                _MyDataAdaptor = new System.Data.SqlClient.SqlDataAdapter(_sqlCommand);
        ...
        public void Add_Parameter_WithValues<T>(string SPInitials, T ModelData) where T : class
        {
            try
            {
                ...
                {
                    // Handle DBNull.Value for nullable types
                    // Added by Pranai Giri - To handle null types
                    if (prop.GetValue(ModelData) == null)
                    {
                        _sqlCommand.Parameters.AddWithValue((prop.Name.IndexOf("@") < 0) ? $"@{SPInitials}{prop.Name}" :
                        SPInitials + prop.Name, DBNull.Value);
                    }
                    else if (prop.PropertyType == typeof(DateTime))
                    {
                        DateTime? dateValue = Convert.ToDateTime(prop.GetValue(ModelData));
        ...
        /// Description :  ExecuteStoredProcedure_GetAsync is an asynchronous method designed to execute a given SQL stored procedure and retrieve its results in a DataTable. 
        ...
        /// </summary>
        public async Task<DataTable> ExecuteStoredProcedure_GetAsync(string spName, object? modelName, string parameterPrefix = "")
        {
            _sqlCommand.CommandText = spName;
            _sqlCommand.CommandType = CommandType.StoredProcedure;
            _sqlCommand.Parameters.Clear();                                          //e.g., like->   sqlCommand.Clear_CommandParameter();
            ...
            if (modelName != null)
            {
                foreach (var property in modelName.GetType().GetProperties())
                {
                    ...
                    _sqlCommand.Parameters.AddWithValue(paramName, paramValue);      //e.g., like->  sqlCommand.Add_Parameter_WithValue("prm_mobilenumber", 9876543210);
                }
            }
        ...
        public async Task<bool> Execute_ScalarAsync(string v, CommandType storedProcedure, SqlParameter[] sqlParameters)
        {
